...
function ghpseoWritingAdditions($name, $plugin_prefix, $domain, $post_set, $page_set, $dashboard_set, $restricted=false, $docounter='') {
$this->__construct($name, $plugin_prefix, $domain, $post_set, $page_set, $dashboard_set, $restricted,$docounter);
return;
}

function __construct($name, $plugin_prefix, $domain, $post_set, $page_set, $dashboard_set, $restricted,$docounter) {
...
function do_meta_dashboard() { // this is crazy, but we need a whole new setup for the dashboard additions as compared to the normal post and page additions, because WordPress doesn't structure its admin pages in a consistent fashion
if ($this->restrict()) return; // if restricted and current user cannot publish posts, don't do anything
